...
void ofApp::setup(){
    ...
    for(int i = 0; i < NUM; i++){
        ...
        speed_x[i] = ofRandom(-10, 10); //x軸方向スピード初期値
        speed_y[i] = ofRandom(-10, 10); //y軸方向スピード初期値
...
void ofApp::update(){
    ...
    for(int i = 0; i < NUM; i++){
        ...
        //もしマウスがクリックされていたらマウスに集ってくる
        if(mouse_pressed){
            //マウスの現在位置から円のスピードを再計算
            speed_x[i] = (mouseX - loc_x[i]) / 8.0; //マウスのx座標と円のx座標の距離の1/8だけ接近
